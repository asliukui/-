...
import com.ly.frame.utils.Utils;
    ...
    private void changeTab(int position) {
        if (mItems.size() > position) {
            for (int i = 0, len = mItems.size(); i < len; i++) {
                ...
                TextView tvName = view.findViewById(R.id.navigation_bar_item_tv_name);
                if (i == position) {
                    view.setEnabled(false);
                    tvName.setSelected(true);
                    tvName.setTextColor(ContextCompat.getColor(mContext,R.color.red));
                } else {
